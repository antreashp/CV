function imOut = compute_LoG(image, LOG_type, varargin)
    switch LOG_type
        case 1
            %% method 1
            % smooth first
            im_smooth = conv2(image, fspecial('gaussian', varargin{1}, varargin{2}), 'same');

            % convolve the image with the laplacian
            imOut = conv2(im_smooth, fspecial('laplacian', 0.5), 'same');

        case 2
            %% method 2
            % convolve directly with the LoG kernel
            imOut = conv2(image, fspecial('log', varargin{1}, varargin{2}), 'same');

        case 3
            %% method 3
            % convolve with the difference of gaussians at different scales
            DoG = fspecial('gaussian', varargin{1}, varargin{3}) - fspecial('gaussian', varargin{1}, varargin{2});
            imOut = conv2(image, DoG, 'same');
    end
end